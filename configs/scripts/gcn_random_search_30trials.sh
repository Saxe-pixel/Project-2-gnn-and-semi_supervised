#!/bin/bash
### Random hyperparameter search for GCN on QM9 (1% labeled / 79% unlabeled), 30 trials per trainer.
#BSUB -q gpuv100
#BSUB -gpu "num=1"
#BSUB -n 4
#BSUB -R "span[hosts=1]"
...
#BSUB -J gcn_random_search_30trials
#BSUB -o results/logs/gcn_random_search_30trials_%J.out
#BSUB -e results/logs/gcn_random_search_30trials_%J.err
...
# Activate your Python environment (adjust if needed)
source ~/venv/gnn311/bin/activate

# Configurable knobs
TRIALS="${TRIALS:-30}"                           # number of samples per trainer
SEED_BASE="${SEED_BASE:-0}"                      # offset for seeds per trial
TOTAL_EPOCHS="${TOTAL_EPOCHS:-80}"
VAL_INTERVAL="${VAL_INTERVAL:-10}"
DATA_DIR="${DATA_DIR:-${REPO_ROOT}/data}"
RESULTS_JSON="${RESULTS_JSON:-${OUTPUT_BASE}/gcn_random_search_30trials_results.json}"
PLOT_PATH="${PLOT_PATH:-${OUTPUT_BASE}/gcn_random_search_30trials_val.png}"
    ...
    for trainer_id, metric_key, display_name in trainers:
        ...
        for trial in range(trials):
            ...
            overrides = [
                ...
                "logger.group=gcn_random_search_30trials",
...
plt.title(f"GCN random search (30 trials per trainer)")
