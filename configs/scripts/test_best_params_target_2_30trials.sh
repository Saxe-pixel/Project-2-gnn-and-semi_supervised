#!/bin/bash
### FINAL EVALUATION: Use best val-MSE configs from gcn_random_search_30trials_results.json
### on the TEST SET for Target 2, to compare with the original gcn_final_test_target2 run.
#BSUB -q gpuv100
#BSUB -gpu "num=1"
#BSUB -n 4
#BSUB -R "span[hosts=1]"
...
#BSUB -J gcn_final_test_target2_best30
#BSUB -o results/logs/gcn_final_test_target2_best30_%J.out
#BSUB -e results/logs/gcn_final_test_target2_best30_%J.err
...
RESULTS_JSON="${RESULTS_JSON:-${OUTPUT_BASE}/gcn_final_test_target2_best30_results.json}"
PLOT_PATH="${PLOT_PATH:-${OUTPUT_BASE}/gcn_final_test_target2_best30_bar.png}"

export REPO_ROOT DATA_DIR RESULTS_JSON PLOT_PATH TOTAL_EPOCHS VAL_INTERVAL TARGET RANDOM_SEARCH_JSON
...
def get_best_entry(display_name: str):
...
# trainer_id, json_display_name, method_label (for output/plot)
trainers = [
    ("semi-supervised-ensemble", "Supervised Ensemble", "Supervised Baseline"),
    ("mean-teacher", "Mean Teacher (EMA)", "Mean Teacher"),
    ("NCPSTrainer", "n-CPS Ensemble", "n-CPS Ensemble"),
]

# Precompute best hyperparameters per trainer from 30-trial random-search results
best_trials = {}
for trainer_id, json_name, method_label in trainers:
    best_entry, overrides = get_best_entry(json_name)
    ...
    print(
        f"Best 30-trial config for {json_name}: "
...
with initialize(config_path="configs", version_base=None):
    for trainer_id, json_name, method_label in trainers:
        ...
        run_name = f"{trainer_id}_final_test_target{target}_best30"
        ...
        print(
            f"Training {method_label} (best 30-trial config) "
            f"| Target {target} | 1% Labeled...",
            flush=True,
        )
        trainer.train(**cfg.trainer.train)
        test_mse = evaluate_test(trainer_id, trainer, device, dm)
        print(f"FINAL TEST MSE (best30): {test_mse:.6f}", flush=True)
...
plt.ylabel("Test MSE")
plt.title(f"Final Test on Target {target} (best 30-trial hyperparameters)")
