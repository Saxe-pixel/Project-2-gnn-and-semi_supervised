#!/bin/bash
### Sweep seeds with fixed 1% labeled / 79% unlabeled splits for all trainers, plot test MSE.
#BSUB -q gpua100
#BSUB -n 4
#BSUB -R "span[hosts=1]"
#BSUB -W 08:00
#BSUB -R "rusage[mem=12GB]"
#BSUB -J gcn_seed_sweep
#BSUB -o batch/logs/gcn_seed_sweep_%J.out
#BSUB -e batch/logs/gcn_seed_sweep_%J.err
...
RESULTS_JSON="${RESULTS_JSON:-${REPO_ROOT}/outputs/gcn_seed_sweep_results.json}"
PLOT_PATH="${PLOT_PATH:-${REPO_ROOT}/outputs/gcn_seed_sweep_loss.png}"

export REPO_ROOT DATA_DIR RESULTS_JSON PLOT_PATH TOTAL_EPOCHS VAL_INTERVAL SEEDS
...
with initialize(config_path="configs", version_base=None):
    for seed in seeds:
        for trainer_id, metric_key, display_name in trainers:
            run_name = f"{trainer_id}_seed{seed}_labels1p"
            overrides = [
            ...
            print(f"Training {display_name} with seed {seed} (1% labeled / 79% unlabeled)...", flush=True)
            trainer.train(**cfg.trainer.train)
            test_mse = evaluate_test(trainer_id, trainer, device, dm)
            print(f"Test MSE ({display_name}, seed {seed}): {test_mse:.6f}", flush=True)

            results[display_name].append({"seed": seed, "test_loss": test_mse})
...
for display_name, entries in results.items():
    entries = sorted(entries, key=lambda x: x["seed"])
    xs = [e["seed"] for e in entries]
    ys = [e["test_loss"] for e in entries]
    plt.plot(xs, ys, marker="o", label=display_name)
plt.xlabel("Seed")
plt.ylabel("Test MSE")
plt.title("GCN seed sweep (1% labeled / 79% unlabeled)")
