...
with initialize(config_path="configs", version_base=None):
    for labeled_pct, unlabeled_pct in zip(label_pcts, unlabel_pcts):
        ...
        for trainer_id, metric_key, display_name in trainers:
            run_name = f"{trainer_id}_labels{str(labeled_pct).replace('.', 'p')}"
            overrides = [
            ...
            print(f"Training {display_name} with {labeled_pct}% labeled / {unlabeled_pct}% unlabeled...", flush=True)
            ...
            results[display_name].append(
                {"labels_pct": labeled_pct, "unlabeled_pct": unlabeled_pct, "test_loss": test_mse}
            )
...
plt.ylabel("Test MSE")
plt.title("GCN label sweep (test loss vs. labeled percentage)")
