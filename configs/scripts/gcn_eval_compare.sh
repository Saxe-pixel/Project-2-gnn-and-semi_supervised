#!/bin/bash
### Compare baseline vs best lambda vs best random-search configs on test MSE across targets/seeds.
#BSUB -q gpua100
#BSUB -gpu "num=1"
#BSUB -n 4
#BSUB -R "span[hosts=1]"
...
#BSUB -J gcn_eval_compare
#BSUB -o results/logs/gcn_eval_compare_%J.out
#BSUB -e results/logs/gcn_eval_compare_%J.err
...
# Configurable knobs
TARGETS="${TARGETS:-0 1 2 3 4 5 6}"          # QM9 targets to evaluate
SEEDS="${SEEDS:-0 1 2}"                      # seeds to average over
TOTAL_EPOCHS="${TOTAL_EPOCHS:-80}"
VAL_INTERVAL="${VAL_INTERVAL:-10}"
DATA_DIR="${DATA_DIR:-${REPO_ROOT}/data}"
RESULTS_JSON="${RESULTS_JSON:-${OUTPUT_BASE}/gcn_eval_compare_results.json}"
PLOT_PATH="${PLOT_PATH:-${OUTPUT_BASE}/gcn_eval_compare_test.png}"
PLOT_VAL_PATH="${PLOT_VAL_PATH:-${OUTPUT_BASE}/gcn_eval_compare_val.png}"
...
# Based on lambda sweep and random search results
configs = [
    # Supervised
    ("semi-supervised-ensemble", "Supervised (baseline)", {}),
    ("semi-supervised-ensemble", "Supervised (best random)", {
    ...
    # Mean Teacher
    ("mean-teacher", "Mean Teacher (lambda=0.005)", {
        "trainer.init.consistency_weight": 0.005,
    }),
    ("mean-teacher", "Mean Teacher (best random)", {
        "model.hidden_channels": 256,
        "model.num_layers": 3,
        "model.dropout": 0.08,
    ...
    # n-CPS
    ("NCPSTrainer", "n-CPS (lambda=0.005)", {
        "trainer.init.cps_weight": 0.005,
    }),
    ("NCPSTrainer", "n-CPS (best random)", {
        "model.hidden_channels": 256,
        "model.num_layers": 5,
        "model.dropout": 0.13,
...
def evaluate(loader, trainer_id, trainer, device, dm):
    ...
    with torch.no_grad():
        if trainer_id == "mean-teacher":
            ...
            for batch, targets in loader:
                ...
                losses.append(torch.nn.functional.mse_loss(preds_denorm, targets_denorm).item())
        else:
...
results = {name: [] for _, name, _ in configs}

with initialize(config_path="configs", version_base=None):
    for target in targets:
        for seed in seeds:
            for trainer_id, display_name, overrides_map in configs:
                safe_name = display_name.replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_")
                run_name = f"{trainer_id}_target{target}_seed{seed}_{safe_name}"
                overrides = [
                ...
                val_mse = evaluate(dm.val_dataloader(), trainer_id, trainer, device, dm)
                test_mse = evaluate(dm.test_dataloader(), trainer_id, trainer, device, dm)
                ...
                # Delete Python references to large objects
                del trainer
                del model
                del models
                del dm
...
# Plot mean test MSE over seeds per target
...
plt.title("Baseline vs tuned configs (1% labeled / 79% unlabeled)")
...
plt.title("Baseline vs tuned configs (validation, 1% labeled / 79% unlabeled)")
plt.grid(True, linestyle="--", alpha=0.4)
plt.legend()
